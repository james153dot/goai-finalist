...
def analyze_live_sample_efficiency() -> dict:
    ...
    for p in payloads:
        ...
        for level in RECALL_LEVELS:
            ai_b = _first_budget_at_level(ai_c, level)
            lhs_b = _first_budget_at_level(lhs_c, level)
            reached[str(level)] = {"ai": ai_b, "lhs": lhs_b}
        per_seed.append(
            {
                "seed": p["seed"],
    ...
    def _band(arr: np.ndarray) -> dict:
        return {
            ...
            "sd": [float(v) for v in np.nanstd(arr, axis=0, ddof=1)],
        }

    ai_band = _band(ai)
    lhs_band = _band(lhs)
    ai_auc = float(np.trapezoid(ai_band["mean"], budgets))
    lhs_auc = float(np.trapezoid(lhs_band["mean"], budgets))
    ...
    payload = {
        ...
        "ai": ai_band,
        "lhs": lhs_band,
        "area_under_mean_recall_curve": {
            "ai": ai_auc,
            "lhs": lhs_auc,
            "difference_ai_minus_lhs": ai_auc - lhs_auc,
            "integration": "trapezoid of mean recall vs budget on [5, 16]",
        },
        "evals_to_recall_level": levels,
        "per_seed": per_seed,
        "claim": (
...
def write_figure(payload: dict | None = None, path: Path = FIG_PATH) -> Path | None:
    ...
    ax.set_title("Sample efficiency on eight live CFD campaigns (n = 8)")
    ax.legend(loc="lower right", fontsize=8)
    ax.text(
        0.02,
        0.96,
        "n = 8 live seeds · no extrapolation past budget 16",
